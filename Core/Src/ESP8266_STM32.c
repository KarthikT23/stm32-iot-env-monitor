...
//    HAL_Delay(5000);  // wait for reset to complete
    ...
    res = ESP_SendCommand("AT+CIPMUX=0\r\n", "OK", 2000); // Force single-connection mode
    if (res != ESP8266_OK){
    	DEBUG_LOG("Failed to set single-connection mode...");
    	return res;
    }
...
ESP8266_Status ESP_SendToThingSpeak(const char *apiKey, float val1, float val2, float val3)
{
    ...
    ESP_SendCommand("AT+CIPCLOSE\r\n", "OK", 1000);  // clear any stale connection, ignore result
    HAL_Delay(500);
    snprintf(cmd, sizeof(cmd), "AT+CIPSTART=\"TCP\",\"api.thingspeak.com\",80\r\n");
    result = ESP_SendCommand(cmd, "CONNECT", 15000);
    if (result != ESP8266_OK) {
